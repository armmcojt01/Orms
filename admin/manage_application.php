	...
	<div class="col-md-12">
		...
		<div class="row form-group">
			<div class="input-group col-md-10 mb-3">
			  ...
			  <div class="custom-file">
			    <input type="file" class="custom-file-input" id="resume" onchangge="displayfname(this,$(this))" name="resume" accept="application/msword,text/plain, application/pdf">
		...
		<div class="row form-group"> 
			<div class="col-md-12"> <!-- Full width -->
				<label for="message" class="control-label" style="margin-top: 20px;">Message</label>
				<textarea name="message" id="" cols="30" rows="3" required class="text-jqte" 
				placeholder="Write your message"><?php echo isset($message) ? $message:''  ?></textarea>
			</div>
		</div>
		...
		$.ajax({
			...
			success: function(resp){
				...
				if (response.status === "success"){
					alert_toast('Application successfully updated.','success');
					setTimeout(function(){
						window.location.href = 'updateEmail.php?email=' + encodeURIComponent(response.email) + '&position_id=' + encodeURIComponent(response.position_id) + '&process_id=' + encodeURIComponent(response.process_id);
					}, 1000);
				} else {
					alert_toast('Update failed. Please try again.', 'error');
				}
			}
		})
