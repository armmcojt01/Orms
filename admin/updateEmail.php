...
if (isset($_GET['email']) && isset($_GET['position_id']) && isset($_GET['process_id'])) {
    $email = $_GET['email'];
    ...
    WHERE a.email = ? AND a.position_id = ? AND a.process_id = ?
    ");
    ...
    $stmt->bind_param("sii", $email, $position_id,$process_id);
    $stmt->execute();
...
function sendStatusUpdateEmail($email, $status_label, $message, $firstname, $lastname, $position_name) {
    ...
    try {
        ...
        $mail->Body = "<p>Dear $firstname $lastname,</p>
                    <p>We are writing to inform you that your application for the position of <strong>$position_name</strong> has been thoroughly evaluated and reviewed.</p>
                    <p><b>Status:</b> <strong>$status_label</strong></p>
                    <p>$message</p>
                    <br>
                    <p>Thank you for your interest in joining our organization. Should you have any questions, please feel free to reach out.</p>
                    <p>Best regards,</p>
                    <p>The HR Team</p>
        ";
